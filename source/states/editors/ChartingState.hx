...
import objects.ChartEditorCharacter;
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	var previewOpp:ChartEditorCharacter = null;
	var previewPlayer:ChartEditorCharacter = null;
	...
	override function create()
	{
		...
		previewOpp = new ChartEditorCharacter(0,0,true);
		previewPlayer = new ChartEditorCharacter(0,0,false);
		...
		previewOpp.cameras = [camUI];
		previewPlayer.cameras = [camUI];

		previewPlayer.x = previewOpp.x + (previewOpp.width*2);
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			for (note in curRenderedNotes)
			{
				...
				if(Conductor.songPosition > note.strumTime && lastTime <= note.strumTime)
				{
					...
					if(note.mustPress){
						if(previewPlayer != null){
							previewPlayer.playAnim(note.songData[1],true,false,0);
							previewPlayer.resetAnim = Math.max(Conductor.stepCrochet * 1.25, note.sustainLength) / 1000 / playbackRate;
							
						}
					}
					else if(!note.mustPress){
						if(previewOpp != null){
							previewOpp.playAnim(note.songData[1],true,false,0);
							previewOpp.resetAnim = Math.max(Conductor.stepCrochet * 1.25, note.sustainLength) / 1000 / playbackRate;
						}
					}
					if(vortexPlaying)
					{
