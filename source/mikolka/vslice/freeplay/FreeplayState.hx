...
import mikolka.vslice.freeplay.DJBoyfriend.FreeplayDJState;
...
class FreeplayState extends MusicBeatSubstate
{
	...
	var beatsOnFour:Int = 0; // for make able to play the idle once every n beats (not using curBeat bc it crashes the game "idk why")

	override function beatHit()
	{
		backingCard?.beatHit(curBeat);

		if (beatsOnFour == 0 || beatsOnFour == 2)
		{
			if (dj.getCurrentState() == FreeplayDJState.Idle){
				dj.playFlashAnimation(dj.getCharData().getAnimationPrefix("idle"),true,false,false,0);
			}
		}
